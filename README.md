...
| `DATABASE_URL` | зарезервирована для слоя БД | Строка подключения к PostgreSQL; текущий сценарий регистрации напрямую БД не использует |
| `AIRTABLE_BASE_ID` | необязательна | Идентификатор базы Airtable |
| `AIRTABLE_TABLE_NAME` | необязательна | Название таблицы Airtable |
| `AIRTABLE_TOKEN` | необязательна локально | Токен персонального доступа Airtable |
| `LOG_LEVEL` | необязательна | Уровень логирования API, по умолчанию `info` |

Без настроенного Airtable приложение работает в режиме разработки: регистрация завершается успешно, но анкета не отправляется во внешнее хранилище.

Чтобы сохранять регистрации в Airtable, задайте все три переменные перед запуском API:
...
Не добавляйте токены и другие секреты в Git.
