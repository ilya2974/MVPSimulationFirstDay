...
const offlineFallbackEnabled = import.meta.env.DEV;
...
const storage = {
  ...
  set(key: string, value: unknown) {
    ...
    const offlineMode = key === 'workday-offline-mode'
      ? value === true
      : storage.get<boolean>('workday-offline-mode', false);
    if (key !== 'workday-profile' && !offlineMode) {
      try {
...
function resetSimulationStorage() {
  try {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      if (key && (key.startsWith('workday-') || key.startsWith('task1') || key.startsWith('simulation'))) {
      ...
      <section className="mx-auto grid min-h-[640px] w-full max-w-[1180px] items-center gap-12 px-6 pb-14 pt-8 md:grid-cols-[1.08fr_.92fr] md:px-10 md:pt-14">
        <div className="animate-rise">
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <button data-testid="button-start-simulation" onClick={() => { resetSimulationStorage(); setLocation('/register'); }} className="button-primary">Начать симуляцию <ArrowRight size={16} /></button>
...
function Register() {
  ...
  const [form, setForm] = useState<Profile>(() => {
    ...
    return { lastName: '', firstName: '', age: '', email: '' };
  });
  const [consent, setConsent] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');
  const update = (key: keyof Profile) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = key === 'age' ? event.target.value.replace(/\D/g, '').slice(0, 2) : event.target.value;
  ...
  const submit = async (event: FormEvent) => {
    ...
    const continueLocally = () => {
      const randomPart = `${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`.slice(-12).padStart(12, '0');
      const participantId = globalThis.crypto?.randomUUID?.() ?? `00000000-0000-4000-8000-${randomPart}`;
      storage.set('workday-profile', { ...form, age: String(ageValue), participantId });
      storage.set('workday-offline-mode', true);
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 503 && offlineFallbackEnabled) {
          continueLocally();
          return;
        }
        ...
        if (result.participantId) {
          ...
          storage.set('workday-offline-mode', false);
          setLocation('/instruction');
          return;
        }
      ...
      if (error instanceof TypeError && offlineFallbackEnabled) {
        continueLocally();
        return;
      }
...
function WindowFrame({ id, title, icon, state, active, onFocus, onClose, onMinimize, onMaximize, children }: { id: AppId; title: string; icon: ReactNode; state: WindowState; active: boolean; onFocus: () => void; onClose: () => void; onMinimize: () => void; onMaximize: () => void; children: ReactNode }) {
  ...
  const dragRef = useRef<{ offsetX: number; offsetY: number; stage: HTMLElement } | null>(null);

  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      const drag = dragRef.current;
      if (!drag) return;
      const windowElement = document.querySelector<HTMLElement>(`[data-testid="window-${id}"]`);
      if (!windowElement) return;
      const stageRect = drag.stage.getBoundingClientRect();
      const windowRect = windowElement.getBoundingClientRect();
      const left = Math.min(Math.max(0, event.clientX - stageRect.left - drag.offsetX), Math.max(0, stageRect.width - windowRect.width));
      const top = Math.min(Math.max(0, event.clientY - stageRect.top - drag.offsetY), Math.max(0, stageRect.height - 44));
  ...
  const startDrag = (event: React.PointerEvent<HTMLElement>) => {
    ...
    const windowElement = titlebar.closest<HTMLElement>('.app-window');
    const stage = titlebar.closest<HTMLElement>('.desktop-stage');
    if (!windowElement || !stage) return;
    const windowRect = windowElement.getBoundingClientRect();
    dragRef.current = { offsetX: event.clientX - windowRect.left, offsetY: event.clientY - windowRect.top, stage };
    const stageRect = stage.getBoundingClientRect();
    setPosition({ left: windowRect.left - stageRect.left, top: windowRect.top - stageRect.top });
    onFocus();
  };

  const style = position ? { left: `${position.left}px`, top: `${position.top}px`, width: positions[id].width, height: positions[id].height, zIndex: state.z, display: state.visible && !state.minimized ? 'flex' : 'none' } : { ...positions[id], zIndex: state.z, display: state.visible && !state.minimized ? 'flex' : 'none' };
  return <section data-testid={`window-${id}`} className={`app-window ${state.maximized ? 'is-max' : ''}`} style={style} onMouseDown={onFocus}><header className={`app-titlebar ${active ? 'active' : ''}`} onPointerDown={startDrag} onDoubleClick={onMaximize}><div className="app-title">{icon}<span>{title}</span></div><div className="window-controls"><button data-testid={`button-minimize-${id}`} className="window-control" onClick={onMinimize} aria-label="Свернуть"><Minus size={14} /></button><button data-testid={`button-maximize-${id}`} className="window-control" onClick={onMaximize} aria-label="Развернуть"><Square size={12} /></button><button data-testid={`button-close-${id}`} className="window-control close" onClick={onClose} aria-label="Закрыть"><X size={14} /></button></div></header><div className="app-content">{children}</div></section>;
}
