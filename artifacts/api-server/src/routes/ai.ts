...
router.post("/ai/chat", async (req, res) => {
  ...
  if (!apiKey) {
    res.status(503).json({ message: "AI-помощник пока не настроен на сервере." });
    return;
  }
  ...
  const messages: ChatMessage[] = [
    {
      role: "user",
      content: "Ты корпоративный ИИ-помощник. Отвечай кратко и структурированно по рабочим задачам первого дня.",
    },
    ...history
      .filter(
  ...
  try {
    ...
    if (result.response.status === 429 && primaryModel !== fallbackModel) {
      result = await requestCompletion(apiKey, fallbackModel, messages);
    }

    const answer = result.data?.choices?.[0]?.message?.content;
    if (!result.response.ok || typeof answer !== "string" || !answer.trim()) {
      res.status(502).json({ message: "Не удалось получить ответ AI-помощника." });
      return;
    }
